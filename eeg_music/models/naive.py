...
class NaiveModel(Module):
    def __init__(
        ...
        nhead: int = 8,
        num_layers: int = 2,  # Number of transformer decoder layers
        dropout: float = 0.1,
        ...
        )  # (batch_size, 32, samples_per_window/2)
        ...
        self.fc2 = torch.nn.Linear(
            128, output_size * 2
        )  # Two Likert scales, each with 5 classes
    ...
    def forward(self, x):
        # (batch_size, n_eeg_channels, samples_per_window)
        x = self.batchnorm(x)  # Apply batch normalization
        x = F.relu(self.conv1(x))  # (batch_size, 16, samples_per_window)
        x = F.relu(self.conv2(x))  # (batch_size, 32, samples_per_window)
        x = self.pool(x)  # (batch_size, 32, samples_per_window//2)

        x = x.permute(2, 0, 1)  # (samples_per_window//2, batch_size, 32)

        tgt = torch.zeros_like(x)

        decoded = self.transformer_decoder(
            tgt, x
        )  # (samples_per_window//2, batch_size, 32)
        decoded = decoded.permute(1, 0, 2)  # (batch_size, samples_per_window//2, 32)
        decoded = decoded.contiguous().view(decoded.size(0), -1)  # Flatten the tensor

        # Fully connected layers with dropout and activation
        decoded = F.relu(self.fc1(decoded))
        decoded = self.dropout(decoded)
        output = self.fc2(decoded)
        output = output.view(-1, 2, 5)  # Reshape to (batch_size, 2, 5)
        return F.softmax(output, dim=2)  # Apply softmax to the last dimension
...
if __name__ == "__main__":
    ...
    model = NaiveModel(batch_size=batch, learning_rate=lr).to(device)
    print(model)
